...
static void i40e_vsi_free_irq(struct i40e_vsi *vsi)
{
	...
	if (pf->flags & I40E_FLAG_MSIX_ENABLED) {
		...
		for (i = 0; i < vsi->num_q_vectors; i++) {
			...
			/* TODO: (BT) When XDP is enabled we should
			 * properly clean up the linked list. However,
			 * when walking the list, incorrect (0)
			 * xQCTL_NEXTQ_INDX are read, so the list
			 * cannot be traversed!
			 */
			  ...
			  struct bpf_prog *prog)
{
	...
	/* TODO: (BT) Always doing a reset is too pessimistic. If
	 * there's a XDP program in place, and we're simply swapping
	 * it, a VSI down/up is enough!
	 */
	i40e_prep_for_reset(pf);
